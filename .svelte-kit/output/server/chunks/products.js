...
const ProductCard = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  })}</div> <span class="text-sm text-gray-600 ml-2">(${escape(product.rating)})</span></div>  <div class="mt-auto"><div class="text-2xl font-bold text-gray-900 mb-3">${escape(formatPrice(product.price))}</div> <button ${product.stock === 0 ? "disabled" : ""} class="w-full btn btn-primary disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2">${validate_component(Shopping_cart, "ShoppingCart").$$render($$result, { class: "h-4 w-4" }, {}, {})} <span class="hidden sm:inline" data-svelte-h="svelte-impvje">Add to Cart</span> <span class="sm:hidden" data-svelte-h="svelte-y6j392">Add</span></button></div></div></div>`;
});
const products = [
  {
    id: 1,
