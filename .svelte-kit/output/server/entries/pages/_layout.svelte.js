...
const Header = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<header class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-50"><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div class="flex justify-between items-center h-16"> <div class="flex items-center" data-svelte-h="svelte-1r7f8zz"><a href="/" class="flex items-center space-x-2" aria-label="TechStore Home"><div class="w-8 h-8 bg-primary-600 rounded-lg flex items-center justify-center"><span class="text-white font-bold text-sm">PC</span></div> <span class="text-xl font-bold text-gray-900">TechStore</span></a></div>  <nav class="hidden md:flex space-x-8" role="navigation" aria-label="Main navigation" data-svelte-h="svelte-1rzow7y"><a href="/" class="text-gray-700 hover:text-primary-600 px-3 py-2 text-sm font-medium transition-colors">Home</a> <a href="/products" class="text-gray-700 hover:text-primary-600 px-3 py-2 text-sm font-medium transition-colors">Products</a> <a href="/about" class="text-gray-700 hover:text-primary-600 px-3 py-2 text-sm font-medium transition-colors">About</a> <a href="/contact" class="text-gray-700 hover:text-primary-600 px-3 py-2 text-sm font-medium transition-colors">Contact</a></nav>  <div class="hidden md:flex flex-1 max-w-lg mx-8"><div class="relative w-full"><div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">${validate_component(Search, "Search").$$render(
    ...
    {},
    {}
  )}</div> <input type="text" placeholder="Search products..." class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-primary-500 focus:border-primary-500" aria-label="Search products"${add_attribute("value", searchQuery, 0)}></div></div>  <div class="flex items-center space-x-4"> <button class="relative p-2 text-gray-700 hover:text-primary-600 transition-colors" aria-label="Shopping cart"${add_attribute("aria-expanded", cartOpen, 0)}>${validate_component(Shopping_cart, "ShoppingCart").$$render($$result, { class: "h-6 w-6" }, {}, {})} ${cartItemCount > 0 ? `<span class="absolute -top-1 -right-1 bg-accent-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">${escape(cartItemCount)}</span>` : ``}</button>  <button class="md:hidden p-2 text-gray-700 hover:text-primary-600 transition-colors" aria-label="Toggle mobile menu"${add_attribute("aria-expanded", mobileMenuOpen, 0)}>${`${validate_component(Menu, "Menu").$$render($$result, { class: "h-6 w-6" }, {}, {})}`}</button></div></div>  <div class="md:hidden pb-4"><div class="relative"><div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">${validate_component(Search, "Search").$$render(
    $$result,
    {
      class: "h-5 w-5 text-gray-400",
      "aria-hidden": "true"
    ...
    {},
    {}
  )}</div> <input type="text" placeholder="Search products..." class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-primary-500 focus:border-primary-500" aria-label="Search products"${add_attribute("value", searchQuery, 0)}></div></div></div>  ${``}</header>`;
});
